...
public class ContainerFileProvider : PhysicalFileProvider, IContainerFileProvider
{
    ...
    public virtual long FileLength(string path) => !FileExists(path) ? -1 : new FileInfo(path).Length;
    ...
    public virtual byte[] ReadAllBytes(string filePath) => File.Exists(filePath) ? File.ReadAllBytes(filePath) : Array.Empty<byte>();
    ...
    public virtual void WriteAllBytes(string filePath, byte[] bytes) => File.WriteAllBytes(filePath, bytes);
